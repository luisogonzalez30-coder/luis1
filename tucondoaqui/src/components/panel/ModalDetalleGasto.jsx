...
// solicitud Resuelta del mes (acotada al mismo filtro de area), con
// horas reales, costo de mano de obra calculado, materiales ítem por ítem
// (no un solo número) y costo final total — para que el Administrador vea EN QUÉ se
...
// (lo que gastó el equipo se alejó del presupuesto) y
...
export default function ModalDetalleGasto({ solicitudes, area, onCerrar }) {
  ...
  return (
    <Modal
      titulo={`Detalle de gasto${area !== 'Todos' ? ` — ${area}` : ''}`}
      onCerrar={onCerrar}
    >
      {totalARevisar > 0 && (
      ...
      {filtradas.length === 0 ? (
        <p className="text-sm text-gray-400">
          {soloRevision ? 'Ninguno de los cierres de este filtro quedó marcado a revisar.' : 'No hay solicitudes resueltas este mes en este filtro.'}
        </p>
     ) : (
        <>
            ...
            {filtradas.map((inc) => {
              ...
              return (
                ...
                >
                  ...
                  {area === 'Todos' && (
                    <p className="text-xs text-gray-400">{inc.area}</p>
                 )}
