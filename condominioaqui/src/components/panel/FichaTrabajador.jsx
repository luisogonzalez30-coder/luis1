import { MapPin, Phone, Mail, Clock, Building2, UserCog } from 'lucide-react'
...
// de ModalPersonalDeArea. Responde las preguntas que el Administrador se
// hace mirando un nombre: de qué area es, de quién depende, si está en
// terreno y dónde exactamente, y cuántas horas lleva.
//
// Todo se calcula sobre datos que el Dashboard ya tiene suscritos (solicitudes,
// roster del área, usuarios) — sin consultas nuevas.
export default function FichaTrabajador({
  ...
  const horas = horasDeTrabajador(trabajador.id, solicitudes)
  const { jefeArea, jefeEquipo } = jefaturaDe({
    area,
    usuarios,
    companeros,
  ...
  return (
    <div className="mt-3 space-y-3 border-t border-black/5 pt-3">
      ...
      {/* Jefatura: dos niveles distintos, y el Administrador necesita los dos. */}
      <div className="space-y-1.5 rounded-xl border border-black/5 px-3 py-2.5">
        ...
        {!jefeArea && !jefeEquipo ? (
          <p className="text-xs text-gray-400">
            Este area no tiene un jefe con cuenta creada todavía.
          </p>
       ) : (
          <>
            <Contacto persona={jefeEquipo} etiqueta="Jefe de equipo" />
            <Contacto persona={jefeArea} etiqueta="Jefe de area" />
          </>
       )}
      </div>
            ...
            {trabajosEnCurso.map((inc) => (
              <li key={inc.id} className="text-xs">
                ...
                <p className="text-gray-600">
                  {inc.ubicacion?.etiqueta || 'Ubicación no registrada'}
                  {inc.direccion_texto && <span className="text-gray-400"> · {inc.direccion_texto}</span>}
                </p>
              </li>
           ))}
          </ul>
