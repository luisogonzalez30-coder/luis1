...
// Presupuesto estimado que el Jefe de Area debe completar ANTES de que
// una solicitud pase a "En Proceso" (ver PanelGestionArea.jsx) — parte
// del módulo de Órdenes de Trabajo y Costeo (ver docs/ARQUITECTURA.md). Solo se usa
// acá; el Administrador sigue asignando equipos sin presupuesto (PanelAsignacion.jsx).
//
...
export default function ModalPresupuesto({ condominioId, area, categoria, solicitudes, onGuardar, onCancelar, guardando }) {
  ...
  // ensucia toda la comparación histórica. Con esto se asigna el equipo
  ...
  useEffect(() => {
    const unsubscribe = suscribirTrabajadores(setTrabajadores, condominioId, area)
    return unsubscribe
  }, [condominioId, area])

  const catalogo = useMemo(() => construirCatalogoMateriales(solicitudes || []), [solicitudes])
  const promedioCategoria = useMemo(
    () => promedioHistoricoPorCategoria(solicitudes || [], categoria),
  ...
  // Con "no aplica" lo único que sigue siendo obligatorio es el equipo: es
  ...
  return (
    <Modal titulo="Presupuesto estimado" onCerrar={onCancelar}>
      <p className="mb-3 text-sm text-gray-500">
        Completa esto antes de asignar el equipo — el ticket pasa a "En Proceso" recién cuando guardes.
      </p>
      ...
      <form onSubmit={manejarSubmit}>
        ...
        {trabajadores.length === 0 ? (
          <p className="mb-3 text-sm text-gray-400">
            Tu area todavía no tiene trabajadores cargados — agrégalos primero en "Mi equipo".
          </p>
       ) : (
        ...
        <label className="toque mb-3 flex items-start gap-2 rounded-xl bg-gray-50 p-3 text-sm ring-1 ring-gray-200">
          ...
          <span>
            ...
            <span className="mt-0.5 block text-xs text-gray-500">
              Para cuando todavía no se sabe el alcance del trabajo. El equipo se asigna igual y el
