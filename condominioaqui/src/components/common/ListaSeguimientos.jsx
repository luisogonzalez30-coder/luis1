...
// Muestra los comentarios/fotos que el residente fue agregando a un reporte ya
// creado (ver ConsultaTicketPage.jsx). Se usa en los 3 paneles de gestión
// (Administrador, Jefe de Area, Terreno) — no se oculta por rol porque, a
// diferencia del contacto del residente, esto es información operativa sobre
// el problema en sí, útil para cualquiera que vaya a atenderlo.
export default function ListaSeguimientos({ solicitudId }) {
  const [seguimientos, setSeguimientos] = useState([])

  useEffect(() => {
    if (!solicitudId) return
    return suscribirSeguimientos(solicitudId, setSeguimientos)
  }, [solicitudId])
  ...
  return (
    <div className="mt-3 rounded-lg border border-gray-200 p-2.5">
      <p className="flex items-center gap-1.5 text-xs font-semibold text-gray-500">
        <MessageSquare size={13} /> Seguimiento del residente ({seguimientos.length})
      </p>
