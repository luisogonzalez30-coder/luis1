...
// Exporta un array de solicitudes a un .csv y dispara la descarga en el navegador.
// Sin librerías nuevas: un CSV simple se abre bien en Excel/Google Sheets.
export function exportarSolicitudesCsv(solicitudes, nombreArchivo = 'solicitudes.csv') {
  const encabezado = COLUMNAS.map((col) => celda(col.titulo)).join(',')
  const filas = solicitudes.map((inc) => COLUMNAS.map((col) => celda(col.obtener(inc))).join(','))
