...
import { calificarSolicitud } from '../services/solicitudesService'
...
import BarraNavegacion from '../components/residente/BarraNavegacion'
...
// Formulario para que el residente agregue más información a un reporte que ya
// existe (ej. "el problema empeoró"). No requiere login — ver agregarSeguimiento.
// Al no poder releer lo enviado (el residente no tiene permiso de lectura sobre
// solicitudes), la confirmación es puramente del lado del cliente tras el envío.
function FormularioSeguimiento({ solicitudId }) {
  ...
  async function manejarEnvio(e) {
    ...
    setError(null)
    try {
      const fotoUrl = archivo ? await subirImagen(archivo, `solicitudes/${solicitudId}/seguimientos`) : ''
      await agregarSeguimiento(solicitudId, { texto: texto.trim(), fotoUrl })
      setEnviado(true)
      setTexto('')
      setArchivo(null)
    } catch (err) {
...
function TarjetaResultado({ resultado, onCalificado }) {
  ...
  async function calificar(estrellas) {
    ...
    try {
      await calificarSolicitud({ solicitudId: resultado.solicitud_id, numeroTicket: resultado.id, calificacion: estrellas })
  ...
  return (
    <div className="mt-5 rounded-xl border border-gray-200 p-4">
      ...
      {resultado.estado === 'Resuelto' && (
        <div className="mt-4 border-t border-gray-100 pt-3">
          {resultado.calificacion_residente ? (
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500">Tu calificación:</p>
              <EstrellasCalificacion valor={resultado.calificacion_residente} soloLectura />
            </div>
         ) : (
            <div className="flex items-center justify-between">
      ...
      <FormularioSeguimiento solicitudId={resultado.solicitud_id} />
    </div>
 )
}

// Página pública ("/estado"), sin login y sin tenant en la URL: el ticket público
// ya trae su propio condominio_id, así que un mismo formulario sirve para
// cualquier condominio. Solo lee de tickets_publicos (campos no sensibles),
// nunca de solicitudes directamente.
//
// Se busca solo por número de ticket. La búsqueda por RUT se eliminó el
// 02-ago-2026 junto con el campo RUT: quien perdió su número lo recupera
// escribiéndole "mis reportes" al WhatsApp de el condominio, y el bot le
// responde únicamente a ese teléfono — más simple para el residente y sin pedirle
// un dato personal sensible.
export default function ConsultaTicketPage() {
  // Solo viene definido en la ruta con condominio ("/:condominioSlug/estado"); en
  ...
  // (el residente no tiene permiso de lectura sobre solicitudes, y el mirror en
  ...
  function manejarCalificado(ticketId, calificacion) {
    setResultado((actual) => (actual?.id === ticketId ? { ...actual, calificacion_residente: calificacion } : actual))
  }
  ...
  return (
    ...
    <div
      className={`mx-auto flex min-h-screen max-w-md flex-col px-4 py-6 ${
        condominioSlug ? 'pb-[calc(var(--alto-barra-inferior)+env(safe-area-inset-bottom,0px))]' : ''
      }`}
    >
      <Link
        to={condominioSlug ? `/${condominioSlug}` : '/'}
        className="mb-4 flex items-center gap-1 text-sm text-tinta-suave hover:text-primary"
      >
        <ArrowLeft size={16} /> {condominioSlug ? 'Volver' : 'Volver al inicio'}
      </Link>
      ...
      {!resultado && (
        <div className="mt-6 flex items-start gap-2 rounded-2xl bg-primary/5 p-4 text-sm text-gray-700">
          ...
          <span>
            ...
            a el condominio desde el mismo teléfono con el que reportaste, y te reenviamos tus
    ...
    {/* La barra solo aparece en la ruta con condominio (/:slug/estado). En "/estado"
        a secas no hay tenant, así que no habría a dónde apuntar las pestañas. */}
    {condominioSlug && <BarraNavegacion condominioSlug={condominioSlug} />}
    </>
 )
}
