...
- **Notificación por WhatsApp al resolver un caso — conversado en detalle el 31-jul-2026, todavía NO implementado.** Decisiones ya tomadas con el usuario, para retomar directo sin volver a preguntar:
  - **Vía elegida: NO oficial** (igual que la tienda del usuario) — automatización tipo "WhatsApp Web" (ej. librería `@whiskeysockets/baileys`) sobre el número **+56977701624**, en vez de la API oficial de Meta (Cloud API). Se le explicó el riesgo real: esto viola los términos de servicio de WhatsApp y el número podría quedar bloqueado sin aviso y sin soporte de Meta — el usuario decidió asumir ese riesgo conscientemente.
  - **Hosting elegido: esta misma computadora** (gratis, pero solo envía si está prendida y conectada en el momento en que se resuelve un caso) — se descartó un servidor pago siempre encendido.
  - **Lo que falta para construirlo**: (1) escanear el QR una vez con el celular de ese número (WhatsApp → Dispositivos vinculados) para autenticar la sesión de Baileys; (2) agregar `notificado_whatsapp: boolean` al esquema de `incidencias` (default `false`) para no notificar dos veces; (3) escribir el script Node persistente (`whatsapp-bot/` o similar) que se loguea como un funcionario dedicado (rol `TERRENO` alcanza — ver §5, `puedeGestionarIncidencia` no restringe TERRENO por departamento ni para lectura), escucha `incidencias` con `estado=='Resuelto' && notificado_whatsapp==false && contacto_ciudadano` con forma de teléfono, manda la foto + mensaje, y marca `notificado_whatsapp: true`.
