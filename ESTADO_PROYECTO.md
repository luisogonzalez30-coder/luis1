...
## 50. TuCondoAquí: la segunda vertical (14-sep-2026)

El motor de TuMuniAquí se abrió para atender un segundo mercado —condominios y edificios—
sin copiar el proyecto ni tocar lo que Licantén tiene en producción. El estudio de mercado
que justifica la decisión está en `docs/ESTUDIO-MERCADO-CONDOMINIOS.md`; el detalle técnico
completo, en `docs/TUCONDOAQUI-ARQUITECTURA.md`. Acá va lo que hay que saber antes de tocar
código.

### 50.1 La idea en una línea

El documento del tenant trae un campo nuevo, `vertical: 'municipio' | 'condominio'`. **Si no
lo trae, es `'municipio'`.** Esa sola decisión es lo que hace que el cambio sea aditivo: no
se migró ni un documento de Licantén, no se tocó el RBAC, no se renombró ningún campo.

`src/verticales/` tiene el registro y los dos paquetes. Cada uno expone la misma interfaz
—`lexico`, `categorias`, `areas`, `ubicacion.modo`, `rolesUI`, `sla`, `calcularGravedad()`,
`calcularArea()`— y quien la consume no sabe cuál es. `verticalDe(tenant)` la resuelve y
tolera `null` (tenant cargando) y valores desconocidos: los dos caen a municipio.

### 50.2 Dos nombres que quedaron cortos y NO se cambiaron

- La colección del tenant se sigue llamando **`municipalidades`**, aunque ahora también
  guarda condominios.
- El área responsable dentro de la incidencia se sigue llamando **`departamento`**, aunque en
  un condominio guarde "Mantención y Obras".

Renombrarlos exigía migrar datos de producción, reescribir `firestore.rules` y volver a
desplegar reglas, a cambio de dos palabras. Está dicho en el código para que nadie lo
"arregle" sin saber el costo.

### 50.3 El catálogo de condominio

`src/verticales/condominio.js`: **84 categorías en 9 grupos**, 12 de ellas marcadas
`legal: true` (obligaciones de la Ley 21.442, que el panel separa del resto porque su atraso
no es un reclamo, es una infracción).

Diferencia de diseño con la vertical municipal: allá el catálogo, el triage y la derivación
viven en tres archivos y hay que acordarse de tocar los tres al agregar una categoría (se
olvidó más de una vez). Acá **cada categoría se declara una sola vez** con sus tres atributos
y los mapas se derivan. Cuando haya que tocar la vertical municipal por otro motivo, conviene
migrarla al mismo formato.

Los colores de grupo siguen la misma regla de la paleta validada (`utils/coloresGrupo.js`): 8
grupos con color en el **orden fijo** de la paleta —el orden es el mecanismo de seguridad para
daltonismo, no algo cosmético— y un noveno neutro.

### 50.4 La ubicación no es un mapa

Un condominio entero cabe dentro del margen de error del GPS: el pin no distingue el piso 3
del 12. El Paso 1 de esta vertical (`components/condominio/PasoUbicacionUnidad.jsx`) es un
selector de dos niveles —primero torre, después unidad— más los espacios comunes, con
buscador para quien ya sabe qué escribir.

La estructura se declara en el tenant, en `torres` y `espacios_comunes`
(`src/utils/unidades.js`). Una torre se puede declarar por `pisos` + `unidades_por_piso` (se
genera 101…1204) o con una lista `unidades` explícita: los conjuntos de casas no siguen
ninguna regla de numeración.

**La incidencia igual guarda `coordenadas`**, con el centro del condominio. No es
redundante: el mapa de calor, el mapa del panel y la regla de Firestore que exige
`coordenadas.lat/lng is number` siguen funcionando sin excepciones por vertical.

### 50.5 Privacidad: la unidad nunca es pública

En un municipio la ubicación de un reporte no identifica a nadie. En un condominio **la
unidad identifica al hogar**. "Torre B · 402 reportó ruidos molestos" en una colección de
lectura pública es publicar quién acusó a quién entre vecinos que comparten el ascensor.

Por eso torre y número **nunca** se copian a `tickets_publicos`; solo viaja el espacio común,
en el campo nuevo `ubicacion_publica`. Consecuencia directa: la agrupación de duplicados
estilo Waze (§16) **solo opera en espacios comunes** —el ascensor detenido que reportan 40
personas— y dentro de una unidad no agrupa nunca. Es una decisión, no un pendiente.

### 50.6 Cumplimiento de la Ley 21.442

`src/utils/mantenciones.js`: 14 obligaciones con periodicidad, responsable, evidencia exigida
y referencia legal, filtradas por lo que el condominio declara tener en `instalaciones`.
Estados: vencida / sin registro / por vencer / al día, con aviso 60 días antes.

Es el diferenciador principal del producto según el estudio de mercado: el software de
condominios que existe en Chile está construido alrededor del dinero, y el vencimiento de una
certificación obligatoria hoy vive en un Excel.

> **Límite que no se cruza:** las periodicidades son valores por defecto editables. El sistema
> lleva el calendario que el administrador configura y avisa antes del vencimiento. **No es
> asesoría legal** y no se vende como tal.

### 50.7 Lo que hay que hacer ANTES de que esto sirva

1. **Desplegar `firestore.rules`** (`npm run desplegar:reglas`). La regla de creación de
   incidencias validaba el campo `departamento` contra la lista fija de los 7 departamentos
   municipales: un reporte de condominio es **rechazado** hasta que se despliegue la nueva
   `areaResponsableValida()`. Este es el paso que, si se olvida, hace que todo parezca roto.
2. **Sembrar el demo**: `npm run condo:demo -- --aplicar` (requiere `serviceAccountKey.json`).
   Sin `--aplicar` solo simula, y corre sin credenciales a propósito.
3. **Panel de Cumplimiento 21.442 en la UI.** La lógica está probada; falta la pantalla. Sin
   pantalla, el diferenciador principal no existe comercialmente.
4. **WhatsApp multi-tenant** (§44): las plantillas de Meta dicen "Municipalidad de Licantén"
   adentro. Bloqueante para el segundo tenant de cualquier vertical.

### 50.8 Riesgo asumido

Nada de esto se pudo ver funcionando en un navegador desde la sesión: el build queda verde y
la simulación del script corre, pero **la comprobación visual sigue siendo del usuario**.
Todos los cambios sobre archivos existentes son aditivos y con valores por defecto que
reproducen el comportamiento municipal anterior, pero eso se verificó leyendo el código y
compilando, no usando la app.
