...
export default function ListaSeguimientos({ incidenciaId }) {
  ...
  return (
    <div className="mt-3 rounded-lg border border-borde p-2.5">
      <p className="flex items-center gap-1.5 text-xs font-semibold text-tinta-suave">
        <MessageSquare size={13} /> Seguimiento del ciudadano ({seguimientos.length})
      </p>
      <ul className="mt-2 space-y-2">
        {seguimientos.map((s) => (
          <li key={s.id} className="text-sm text-tinta">
            {s.texto && <p>{s.texto}</p>}
            {s.foto_url && (
            ...
            <p className="text-xs text-tinta-tenue">{formatearFecha(s.fecha)}</p>
          </li>
        ))}
      </ul>
    </div>
