...
export default function UltimosReportes({ reportes }) {
  ...
  return (
    <div>
      <h3 className="mb-2 text-sm font-semibold text-tinta">Últimos reportes de la comuna</h3>
      <ul className="divide-y divide-borde overflow-hidden rounded-2xl border border-borde bg-white shadow-sm">
        {reportes.map((r) => {
          ...
          return (
            <li key={r.id}>
              <button
                ...
                className="flex w-full items-center gap-2 px-3 py-2.5 text-left transition-colors hover:bg-slate-50 active:bg-slate-100"
              >
                <span
                ...
                <span className="shrink-0 text-xs text-tinta-tenue">{tiempoRelativo(r.fecha_creacion)}</span>
                <BadgeEstado estado={r.estado} />
                <ChevronRight size={16} className="shrink-0 text-tinta-tenue" />
              </button>
            </li>
          )
        })}
