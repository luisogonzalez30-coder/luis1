...
4. **Corregir la variable `URL_BOT`** en <https://github.com/luisogonzalez30-coder/luis1/settings/variables/actions> (pestaña **Variables**, NO "Secrets"). La creaste el 21-ago, pero con `https://tumuniaqui-bot.onrender.com`, que **no existe**: ese nombre lo puso esta guía como ejemplo inventado y se copió literal. El servicio real es **`https://proyectomuni.onrender.com`** (sin barra al final) — es la dirección que aparece en §39 de ESTADO_PROYECTO.md y en la pauta de reunión. Render responde 404 en cualquier subdominio suyo sin dueño, así que una URL inventada se ve exactamente igual que un servicio caído; por eso el vigilante lleva fallando desde que se encendió y el aviso #6 está abierto. **Antes de guardarla**, se puede probar sin tocar nada: pestaña Actions → "Vigilar el servicio" → **Run workflow**, pegando la dirección en el campo *url_bot*. Esa corrida revisa esa URL y no abre ni cierra el aviso real. Va como variable y no como secreto a propósito: la URL no es secreta, y siendo variable se ve en los registros, que es justo cuando hace falta leerla.

   Si con la dirección correcta sigue dando 404, entonces el servicio existe pero corre una versión anterior a `/salud`: hay que redesplegarlo en Render desde `main` (el endpoint entró con el merge del PR #5, el 21-ago).
