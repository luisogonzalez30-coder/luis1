...
4. **Crear la variable `URL_BOT`** en <https://github.com/luisogonzalez30-coder/luis1/settings/variables/actions> — pestaña **Variables**, NO "Secrets" —, con la dirección del bot en Render y sin barra al final (algo como `https://tumuniaqui-bot.onrender.com`). Es lo único que le falta al vigilante (`.github/workflows/vigilar.yml`) para quedar completo: sin ella revisa solo el sitio web y anota en el registro que no pudo revisar el bot; con ella vigila las dos cosas cada 30 minutos y, si algo se cae, **abre un issue en este repositorio** que GitHub reenvía por correo. Va como variable y no como secreto a propósito: la URL no es secreta, y siendo variable se ve en los registros, que es justo cuando hace falta leerla.
